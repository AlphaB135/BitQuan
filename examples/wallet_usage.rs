...
use wallet::keystore::{
    encrypt_keystore_adaptive, decrypt_keystore,
    WalletConfig, encrypt_keystore_with_config, decrypt_keystore_with_config,
    get_cache_stats, get_cache_memory_usage, cleanup_expired_cache
};
...
fn cache_monitoring_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Memory usage: {} bytes ({} KB)", memory_bytes, memory_bytes / 1024);
