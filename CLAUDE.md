...
-   **Check git history first** (2026-01-18): Always run `git log --oneline -5` BEFORE diving into file reading; recent commits tell you what's already done - saves redundant work
-   **`#[allow(dead_code)]` vs deletion** (2026-01-18): For potentially useful code (metrics servers), use `#[allow(dead_code)]` instead of deleting; keeps options open for future use
-   **Test expectations vs implementation** (2026-01-18): When test fails due to expectation mismatch, update test to match REALITY not the other way around; hex encoding vs argon2 - trust the code
...
-   **P2P Integration Test Pattern** (2026-01-18): Background process testing: `cargo run ... > /tmp/log 2>&1 &` then monitor logs; Node A: `p2p-server --listen 127.0.0.1:18444`; Node B: `p2p-server --listen 127.0.0.1:18445 --connect 127.0.0.1:18444`
-   **Async P2P Migration Verification** (2026-01-18): `tokio::net::TcpListener` + `--connect` flag = successful handshake; Look for "Incoming connection from..." on server, "Connected to...! ✅" on client
-   **Pre-launch Checklist** (2026-01-18): BEFORE mainnet: (1) Maturity 10→100+, (2) Remove dummy JWT secrets, (3) Verify difficulty settings; These are launch blockers, not nice-to-haves
...
-   **Parallel Agents for Comprehensive Audit** (2026-01-20): Launch 3 Task agents simultaneously (mining, wallet, transactions) for full system audit; 66% time reduction vs sequential; Coverage exponentially better with specialized domain experts; Use when verifying complex multi-component features
-   **Reuse Existing Infrastructure Before Adding Dependencies** (2026-01-20): Bincode 2.0 API confusing - tried 4 approaches before discovering existing `serialize` module in storage crate; Always check `pub mod` exports and `lib.rs` re-exports before coding; Export > Import > Implement
-   **Test Reality Match Principle** (2026-01-20): Mock storage used JSON, production used bincode → test failures; Tests MUST use same data structures as production; `StoredUtxoEntry` with `serialize::to_bytes` in both; Tests validate production, not fantasies
-   **Coinbase Maturity Storage Pattern** (2026-01-20): UTXOs must store `height` and `is_coinbase` for validation; Undo data preserves maturity during reorgs; `StoredUtxoEntry { output, height, is_coinbase }` format; 100-block validation uses `saturating_add` for overflow safety
