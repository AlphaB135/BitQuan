...
use wallet::keystore::{
    WalletConfig, encrypt_keystore_with_config, decrypt_keystore_with_config,
    get_cache_stats, get_cache_memory_usage
};
use bitquan_types::{
    TxContext, Transaction, TxIn, TxOut, NetworkId, SigAlgorithm,
};
...
fn test_end_to_end_pqc_signature_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Decrypt private key
    let decrypted_key = decrypt_keystore_with_config(&keystore, password, &server_config)?;
...
fn test_multithreaded_signing_with_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let performance_config = WalletConfig::performance()
        .with_cache_timeout(Duration::from_secs(30));
    ...
    for (thread_id, signature) in signatures {
        ...
        assert!(verification.is_ok(), "Thread {} signature verification failed", thread_id);
    }
...
fn test_cache_timeout_and_cleanup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _decrypted1 = decrypt_keystore_with_config(&keystore, password, &short_timeout_config)?;
    let time1 = start1.elapsed();
    
    let stats1 = get_cache_stats();
    println!("  ✅ First decryption: {:?}, Cache entries: {}", time1, stats1.active_entries);
    
    // Second decryption immediately (should use cache)
    println!("  ⚡ Second decryption (should use cache)...");
    let start2 = Instant::now();
    let _decrypted2 = decrypt_keystore_with_config(&keystore, password, &short_timeout_config)?;
    let time2 = start2.elapsed();
    
    let stats2 = get_cache_stats();
    println!("  ✅ Second decryption: {:?}, Cache entries: {}", time2, stats2.active_entries);
    ...
    println!("  📊 Cache status: {} total, {} expired", stats3.total_entries, stats3.expired_entries);
    
    // Third decryption (should still use cache)
    println!("  🔄 Third decryption (should still use cache)...");
    let start3 = Instant::now();
    let _decrypted3 = decrypt_keystore_with_config(&keystore, password, &short_timeout_config)?;
    let time3 = start3.elapsed();
    
    println!("  ✅ Third decryption: {:?}", time3);
    
    // Verify cache is still working
    println!("  🚀 Cache performance: Cold={:?}, Hot={:?}, Hot2={:?}", time1, time2, time3);
...
fn test_error_handling_and_security() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match decrypt_keystore_with_config(&keystore, "wrong-password", &server_config) {
    ...
    let decrypted = decrypt_keystore_with_config(&keystore, correct_password, &server_config)?;
