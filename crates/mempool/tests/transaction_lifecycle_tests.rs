...
use bitquan_types::{Transaction, TxIn, TxOut, Witness, NetworkId, SignaturePayload};
...
fn test_mempool_size_tracking() {
    ...
    assert!(after_size > initial_size, "size should increase after insertion");
}
...
fn test_mempool_size_limit() {
    ...
    mempool.insert(tx.clone(), 1000).expect("first insert should succeed");
    ...
    assert!(inserted_count < 9, "size limit should prevent all insertions");
}
