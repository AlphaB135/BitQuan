...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::DnsCheck { network, timeout: timeout_secs } => {
            dns_check(&network, timeout_secs).await?;
        }
        Commands::TcpProbe { host, port, timeout_ms } => {
...
async fn dns_check(network: &str, timeout_secs: u64) -> Result<()> {
    ...
    let seeds_content = fs::read_to_string(&seeds_file)
        .context("Failed to read dns_seeds.txt")?;
    
    let pattern = if network == "testnet" { "testnet" } else { "seed" };
    ...
    for seed in &seeds {
        ...
        results.push(SeedStatus {
            seed: seed.clone(),
            reachable,
            error: if reachable { None } else { Some("Unreachable".to_string()) },
        });
    }
...
mod tests {
    ...
    async fn test_tcp_probe_localhost() {
        ...
        let reachable = check_tcp_reachability(
            "localhost",
            9999,
            Duration::from_millis(100)
        ).await;
        
        // Should be false unless something is actually running on 9999
