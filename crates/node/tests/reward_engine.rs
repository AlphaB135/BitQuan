...
use bitquan_node::{
    BlockRecord, ChainState, MiningMetrics, PoolDatabase, RewardEngine,
};
...
fn test_multiple_miners() {
    ...
    engine.record_block(&dummy_block(0), [1u8; 32], 0, "alice").unwrap();
    engine.record_block(&dummy_block(1), [2u8; 32], 1, "bob").unwrap();
    engine.record_block(&dummy_block(2), [3u8; 32], 2, "alice").unwrap();
    
    let alice_reward = engine.get_miner_reward("alice").unwrap();
    let bob_reward = engine.get_miner_reward("bob").unwrap();
    
    assert_eq!(alice_reward, 50_0000_0000 * 2, "Alice should have 2x rewards");
...
fn test_payout_recording() {
    ...
    // Record a payout
    let payout_id = engine.record_payout("miner1", 1000000, Some("tx123".to_string())).unwrap();
...
fn test_database_persistence() {
    ...
    // Scope 1: Create and populate database
    {
        ...
        engine.record_block(&dummy_block(0), [1u8; 32], 0, "miner1").unwrap();
        engine.record_block(&dummy_block(1), [2u8; 32], 1, "miner1").unwrap();
    }
