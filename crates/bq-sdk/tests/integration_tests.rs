...
use bq_sdk::{Address, AddressType, DerivationPath, Mnemonic, Network, SignatureAlgorithm, PQPSBT};
...
fn test_wallet_configs() {
    ...
    assert!(
        desktop_config.performance.max_cache_entries > mobile_config.performance.max_cache_entries
    );
}
...
fn test_serialization_roundtrip() {
    ...
    assert_eq!(
        ...
        deserialized.signature_algorithms
    );
    assert_eq!(
        config.security.quantum_entropy,
        deserialized.security.quantum_entropy
    );
}
